...
import com.gypso.gypsogest.repository.ItemRepository;
...
/**
 * REST controller for managing {@link com.gypso.gypsogest.domain.Item}.
 */
@RestController
...
public class ItemResource {
    ...
    public ResponseEntity<Item> partialUpdateItem(
        ...
        Optional<Item> result = itemRepository
            .findById(item.getId())
            .map(existingItem -> {
                if (item.getGypseType() != null) {
                    existingItem.setGypseType(item.getGypseType());
                }
